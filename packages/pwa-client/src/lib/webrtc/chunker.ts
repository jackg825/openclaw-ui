import type { ChunkEnvelope } from '@shared/webrtc-signaling';
...
export class MessageChunker {
  ...
  receive(raw: string): string | null {
    ...
    try {
      ...
      return raw; // Not valid JSON, pass through as-is
    }

    const obj = parsed as Record<string, unknown>;
    if (!obj._chunk) return raw; // Not a chunk envelope, pass through
  ...
  /** Clear any incomplete pending chunks */
  clear(): void {
    this.pending.clear();
  }
}
